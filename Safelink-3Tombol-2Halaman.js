...
function AnimeButton(TimeLoading, IdButton, ColorButton, TextStart, TextEnd, FunctionButton, NextFunction) {
    var getPlay = 0;
    getFunct1();

    function getFunct1() {
        if ((getPlay += 1) < 11) {
            document.getElementById(IdButton).setAttribute('style', `background: linear-gradient(90deg, ` + ColorButton + ` ` + (getPlay * 10) + `%, #cccccc 1%);`);
            document.getElementById(IdButton).innerHTML = TextStart + (getPlay * 10) + '%';
            document.getElementById(IdButton).disabled = true;
            if (getPlay == 10) {
                document.getElementById(IdButton).innerHTML = TextEnd;
                document.getElementById(IdButton).removeAttribute('disabled');
                document.getElementById(IdButton).setAttribute('onclick', FunctionButton);
                executeFunctionByName(NextFunction);
            }
            setTimeout(function () {
                getFunct1();
            }, (TimeLoading * 1000) / 10);
...
function AnimeInput(TimeLoading, IdButton, ColorButton, TextStart, TextEnd) {
    document.getElementById(IdButton).value = TextStart;
    document.getElementById('CopyText').innerHTML = 'Plase Wait...';
    ...
    function getFunct2() {
        if ((getPlay += 1) < 11) {
            document.getElementById(IdButton).setAttribute('style', `background: linear-gradient(90deg, ` + ColorButton + ` ` + (getPlay * 10) + `%, #cccccc 1%);`);
            if (getPlay == 10) {
                document.getElementById(IdButton).value = TextEnd;
                document.getElementById('iconNotifGetLink').innerHTML = '<i class="far fa-check-circle"></i>';
                document.getElementById('NotifGetLink').innerHTML = 'Succes';
                document.getElementById('CopyText').innerHTML = `<span id='iconCopy'><i class="far fa-copy"></i></span>
...
function CopyText() {
    document.getElementById('TexButtonCopy').innerHTML = 'Please Wait..';
    document.getElementById('panelCopy').disabled = false;
    document.getElementById('panelCopy').select();
    document.getElementById('iconCopy').innerHTML = '<i class="fas fa-cog fa-spin"></i>';
    try {
        var successful = document.execCommand('copy');
        setTimeout(function () {
            document.getElementById('TexButtonCopy').innerHTML = 'Copied';
            document.getElementById('iconCopy').innerHTML = '<i class="far fa-check-circle"></i>';
            document.getElementById('panelCopy').disabled = true;
        }, 500);
    } catch (err) {}
}
...
if (window.location.href.indexOf(PathPage1) > 0) {
    if (document.getElementById('boxstart1')) {
        document.getElementById('boxstart1').innerHTML = '<span id="myButton1" disabled></span>';
        var getLinkDefault = window.location.href.split(PathPage1)[0];
        var replaceurl = getReplaceUrl();
        window.history.replaceState({}, document.title, getLinkDefault);
        var getLinkEncrypt = replaceurl.split(PathPage1)[1];
        AnimeButton(TimeLoadingButtonFirst, 'myButton1', ColorButtonFirst, TextStartButtonFirst, TextEndButtonFirst, 'getLink1();', 'getVisit1');
        var cek_listgeturl = '';
        function getVisit1() {
            var lunkkk = '//' + window.location.hostname + "/feeds/posts/summary/?alt=json-in-script&orderby=updated&max-results=9999";
            var listgeturl = false;
            var listgeturl2 = false;
            if (cek_listgeturl.length == 0) {
                $.ajax({
                    url: lunkkk,
                    type: 'get',
                    dataType: 'jsonp',
                    success: function aku(datajsongenerate) {
                        var datafeed = datajsongenerate.feed;
                        var jumlahurl = datafeed.openSearch$totalResults.$t;
                        if (jumlahurl > 150) {
                            var totalhurl = 150;
                        };
                        if (jumlahurl <= 150) {
                            var totalhurl = jumlahurl;
                        };
                        var listgenurl = new Array();
                        for (var i = 0; i < totalhurl; i++) {

                            if (datafeed.entry[i].link[4] === undefined) {
                                var akuula = datafeed.entry[i].link[2].href;
                                if (akuula.indexOf(window.location.hostname) >= 0) {
                                    listgenurl[i] = datafeed.entry[i].link[2].href;
                                }
                            } else {
                                listgenurl[i] = datafeed.entry[i].link[4].href;
                            }
                        }
                        listgeturl = listgenurl;
                        cek_listgeturl = listgenurl;
                    },
                    async: false
                });
            }
        ...
        function getLink1() {
            var outputgenerate = cek_listgeturl[parseInt(Math.random() * cek_listgeturl.length)] + PathPage2 + getLinkEncrypt;
            let a = document.createElement('a');
            a.target = '_self';
            a.href = outputgenerate;
            a.click();
        }
    } else {
        console.log('No element found with id tag "boxstart1"')
    }
}
if (window.location.href.indexOf(PathPage2) > 0) {
    if (document.getElementById('stoped1')) {
        document.getElementById('stoped1').setAttribute('style', 'height: ' + HeightScroolToButton2 + 'px;');
    }
    if (document.getElementById('stoped2')) {
        document.getElementById('stoped2').setAttribute('style', 'height: ' + HeightScroolToBoxCopy + 'px;');
    }
    if (document.getElementById('boxstart1')) {
        document.getElementById('boxstart1').innerHTML = '<span id="myButton1" disabled>Get Link</span>';
        document.getElementById('boxstart2').innerHTML = '<span id="myButton2" disabled>Next GoTo Link</span>';
        document.getElementById('boxstart3').innerHTML = `
                        ...
                        `;
        document.getElementById('boxstart2').setAttribute('class', 'none');
        document.getElementById('boxstart3').setAttribute('class', 'none');
        var getLinkDefault = window.location.href.split(PathPage2)[0];
        var replaceurl = getReplaceUrl();
        window.history.replaceState({}, document.title, getLinkDefault);
        var getLinkEncrypt = replaceurl.split(PathPage2)[1];
        var getLinkDecrypt = aesCrypto.decrypt(convertstr(getLinkEncrypt), convertstr('root'));
        if (getLinkDecrypt.split('').length > 0) {
            if (getLinkDecrypt.indexOf('DirectFrom:') > 0) {
                AnimeButton(TimeLoadingButton1, 'myButton1', ColorButton1, TextStartButton1, TextEndButton1, 'geToButton2();', 'getVisit2');
                var getDirectFrom = getLinkDecrypt.split('DirectFrom:')[1];
                var getLinkDecryptFix = getLinkDecrypt.split('DirectFrom:')[0];
                document.getElementById('NameFile').innerHTML = getDirectFrom;

                function getVisit2() {
                    if (document.getElementById('boxstart2')) {
                        document.getElementById('boxstart2').innerHTML = '<span id="myButton2" disabled>...</span>';
                    } else {
                        console.log('No element found with id tag "boxstart2"')
                    }
                }

                function geToButton2() {
                    AnimeButton(TimeLoadingButton2, 'myButton2', ColorButton2, TextStartButton2, TextEndButton2, 'geToButton3();', 'getVisit3');
                    document.getElementById("stoped1").scrollIntoView({
                        behavior: "smooth"
                    });
                    document.getElementById('boxstart2').removeAttribute('class');
                    document.getElementById('boxstart1').setAttribute('class', 'none');
                    //document.getElementById("boxstart1").remove();
                }

                function getVisit3() {
                    if (document.getElementById('boxstart3')) {

                    } else {
                        console.log('No element found with id tag "boxstart3"')
                    }
                }

                function geToButton3() {
                    document.getElementById("stoped2").scrollIntoView({
                        behavior: "smooth"
                    });
                    AnimeInput(TimeLoadingBoxCopy, 'panelCopy', ColorBoxCopy, getLinkEncrypt, getLinkDecryptFix);
                    document.getElementById('boxstart3').removeAttribute('class');
                    document.getElementById('boxstart2').setAttribute('class', 'none');
